...
FOR %%x IN ("C:\Users\Quinn\Projects\HollyHug\PHP5-CLI\pear") DO (if %%x=="%PMTPMTPMT%" GOTO :NOTINSTALLED)

REM Check PEAR global ENV, set them if they do not exist
IF "%PHP_PEAR_INSTALL_DIR%"=="" SET "PHP_PEAR_INSTALL_DIR=C:\Users\Quinn\Projects\HollyHug\PHP5-CLI\pear"
IF "%PHP_PEAR_BIN_DIR%"=="" SET "PHP_PEAR_BIN_DIR=C:\Users\Quinn\Projects\HollyHug\PHP5-CLI"
IF "%PHP_PEAR_PHP_BIN%"=="" SET "PHP_PEAR_PHP_BIN=C:\Users\Quinn\Projects\HollyHug\PHP5-CLI\.\php.exe"
GOTO :INSTALLED
...
GOTO END
:RUN
"%PHP_PEAR_PHP_BIN%" -c .\php.ini-recommended -C -d output_buffering=1 -d safe_mode=0 -d open_basedir="" -d auto_prepend_file="" -d auto_append_file="" -d variables_order=EGPCS -d register_argc_argv="On" -d include_path="%PHP_PEAR_INSTALL_DIR%" -f "%PHP_PEAR_INSTALL_DIR%\pearcmd.php" -- %1 %2 %3 %4 %5 %6 %7 %8 %9
:END
@ECHO ON