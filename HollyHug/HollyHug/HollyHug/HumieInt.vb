...
Public Class HumieInt
    ...
    Private Sub Setup()
        ...
        If Not TwoDesks Then
            ...
            mtx = Round((s1x * mmThresh))
            mty = Round((s1y * mmThresh))
        Else
            ' FOR TWO DISPLAYS:
            ' WIDTH  is based upon the total width (added) of the two desktops
            mtx = Round(((s1x + s2x) * mmThresh))
            ...
            If s2y > s1y Then
                ...
                mty = Round((s2y * mmThresh))
            Else
                ' Desktop ONE is bigger, OR is SAME HEIGHT as Desktop TWO ... Use height of Desktop ONE...
                mty = Round((s1y * mmThresh))
            End If
        End If
        LogPrint("STARTUP: Mouse X Threshold (" + Str((mmThresh * 100)) + "%) is " + Str(mtx) + " pixels!")
        LogPrint("STARTUP: Mouse Y Threshold (" + Str((mmThresh * 100)) + "%) is " + Str(mty) + " pixels!")
    ...
    Public Sub RunCheck()
        ...
        If Not ItsAGame() And HollyHug.UseSmartPowerToolStripMenuItem.Checked And Not ServiceCheckingModule.RunCheck("eHShell") And Not ServiceCheckingModule.RunCheck("iTunes") Then
            ...
            If Not SmartPower Then
                ...
                If IdleRuns > NapTicks And ServiceCheckingModule.HadSaver = True Then
                    SmartPower = True
                ...
                End If
            Else
                If ServiceCheckingModule.HadSaver = False Or IdleRuns = 0 Then
                    SmartPower = False
